...
rpsApp.config(["$routeProvider", "$locationProvider", "$httpProvider", function($routeProvider, $locationProvider, $httpProvider) {
...
rpsApp.controller("rpsCtrl", ["$scope", "$timeout", "$http", "$location", "$route", "$templateCache", function($scope, $timeout, $http, $location, $route, $templateCache) {
    ...
    var tmpObjStats = {};
     ...
     // $http({method:'GET', url:'http://127.0.0.1:1337', params: {'id':'id','up': false}}).
     //         success(function(data) {
     //             console.log(data);
     //        });
   // function statsFromServer(){
   	// var conf = {headers: {origin: "http://127.0.0.1:1337"}};
    	$http.post("http://127.0.0.1:1337", "", {
    		...
    		success(function(data, status, headers, config) {
                // tmpObjStats=data.question;
              console.log(data);
            }).error(function(data, status, headers, config) {
                console.log("код відповіді: " + status);
                console.log(headers);
            });
   	//}
    //}
